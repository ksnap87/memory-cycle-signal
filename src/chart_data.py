...
def data_recent_exports(df: pd.DataFrame) -> dict:
    # 반도체(전체) ⊃ 메모리 ⊃ 디램·낸드 — 한 그래프에 겹쳐 구성비를 보이게.
    cols = [("반도체수출_8542_백만$", "반도체 전체", "#0071e3"),
            ("메모리수출_854232_백만$", "메모리(D램+낸드)", "#ff3b30"),
            ("디램수출_백만$", "디램(DRAM·HBM 포함)", "#34c759"),
            ("낸드수출_백만$", "낸드(NAND)", "#ff9500")]
    idx = None
    for col, _, _ in cols:
        if col in df.columns:
